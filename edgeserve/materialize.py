...
class Materialize(Loggable):
    def __init__(self, materialize, pulsar_node, gate=None, ftp_in=False, ftp_delete=False,
                 local_ftp_path='/srv/ftp/', topic='dst', log_path=None, log_filename=None, is_overhead_logged=False):
        self.pulsar_node = pulsar_node
    ...
    def __next__(self):
        ...
        if self.ftp_in:
            ...
            self.p2p_log(msg_in_uuid, op_from, local_file_path)
        ...
        if self.log_path and os.path.isdir(self.log_path):
            ...
            with open(log_file, 'a') as f:
                f.write(str(msg_in_uuid) + ',' + op_from + ',' + str(output) + ',' + str(task_finish_time_ms) + '\n')
